    ...
    init = (amgn, amn, checkid, options = {}) => {
        ...
        if (!this._inited || (!this._checkid) || (checkid != this._checkid)) {
            return new Promise((resolve, reject) => {
                this.request(amn, "reg_am", act, null, null, options, resolve, reject);  // ,
																							// null,
																							// null,
																							// options
            });
            ...
            return new Promise((resolve, reject) => {
                this.request(amn, "call", act, null, null, options, resolve, reject);  // ,
																						// null,
																						// null,
																						// options
            });
...
// }
    ...
    call = (amn, name, ados, jsondata, options) => {
        ...
        return new Promise((resolve, reject) => {
            this.request(amn, "call", name, ados, jsondata, options, resolve, reject);  // ,
																						// null,
																						// null,
																						// options
        });
    ...
    selfCall = (amn, name, ados, jsondata, options) => {
        return new Promise((resolve, reject) => {
            this.request(amn, "async", name, ados, jsondata, options, resolve, reject);  // ,
																							// null,
																							// null,
																							// options
        });
            ...
            // 必须要处于同一个group和_checked;
                ...
                if (toparent) {
                	...
                	if (this.parentChannel){
                		this.parentChannel.emit('callback',options);
                	}
                } else {
    ...
    ajax = (urlParams, postData, options, resolve, reject) => {
        ...
        fly.request(this.serialURL(urlParams), postData, {
            method: 'POST'
        }).then((res) => {
            if (res.status === 200) {
                try {
